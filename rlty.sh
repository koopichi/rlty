...
image[sing-box]="gzxhwq/sing-box:v1.3.6"
...
function show_help {
  echo ""
  echo "Usage: rlty.sh [-t|--transport=tcp|http|grpc|ws] [-d|--domain=<domain>] [--server=<server>] [--regenerate] [--default]
...
function tune_kernel {
  cat >/etc/sysctl.d/99-rlty.conf <<EOF
fs.file-max = 200000
...
EOF
  sysctl -qp /etc/sysctl.d/99-rlty.conf >/dev/null 2>&1 || true
}
